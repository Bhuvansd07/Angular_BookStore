...
export class AppComponent {
  ...
  cartItems: Array<CardDetails> = []
  total: number = 0
  quantity: number = 0

  insertBook(e:any){
    // this.cartItems.push(e);
    let index = this.cartItems.findIndex(obj => obj.ISBN === e.ISBN);
    if(index === -1)
    {
      let currentBook : CardDetails = {
        ISBN: e.ISBN,
        title: e.title,
        image: e.image,
        price: e.price.value,
      ...
      this.cartItems.push(currentBook);
    }
    else{
      this.cartItems[index].quantity += 1;
    }
    this.total = parseFloat((this.total + e.price.value).toFixed(2));
  } 

  removeItem(ISBN:number){
    let itemIndex = this.cartItems.findIndex(obj => obj.ISBN === ISBN);
    this.total = parseFloat((this.total - this.cartItems[itemIndex].quantity*this.cartItems[itemIndex]?.price).toFixed(2));
    this.cartItems.splice(itemIndex,1);
  }

  increaseQuantity(ISBN: number)
  {
    let index = this.cartItems.findIndex(obj => obj.ISBN === ISBN);
    this.cartItems[index].quantity += 1;
    this.total = parseFloat((this.total + this.cartItems[index].price).toFixed(2))
  }

  decreaseQuantity(ISBN: number)
  {
    let itemIndex = this.cartItems.findIndex(obj => obj.ISBN === ISBN);
    this.total = parseFloat((this.total - this.cartItems[itemIndex].price).toFixed(2));
